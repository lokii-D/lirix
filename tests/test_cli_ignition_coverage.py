...
def test_test_v15_cli_ignition(tmp_path: Path) -> None:
    ...
    assert "LIRIX_HOOK_CONTRACT_MODE=shadow" in env_text
    assert "LIRIX_POLICY_LIFECYCLE_MODE=digest_verified" in env_text
    assert "LIRIX_RPC_EVIDENCE_MODE=v2_only" in env_text
...
def test_test_v15_cli_ignition_2(tmp_path: Path) -> None:
    ...
    assert env_text.count("LIRIX_HOOK_CONTRACT_MODE=") == 1
...
def test_test_v15_cli_ignition_7(tmp_path: Path) -> None:
    ...
    assert "from typing import Dict, List, cast" in generated
    assert "_governance_modes_from_env" in generated
    assert "from lirix import Lirix" in generated
    assert "run_guardian" in generated
